...
from hotpot.nn.layers import Mapper, SequenceMapper


class ElmoLayer(Mapper):
    def __init__(self, l2_coef: float, layer_norm: bool, top_layer_only: bool, coef_init_array=None,
                 gamma_val=1.0, trainable_coefs=True):
        self.l2_coef = l2_coef
        self.layer_norm = layer_norm
        self.top_layer_only = top_layer_only
        self.coef_init_array = coef_init_array
        self.gamma_val = gamma_val
        self.trainable_coefs = trainable_coefs

    def apply(self, is_train, x, mask=None):
        coef_init = None if self.coef_init_array is None else tf.constant_initializer(self.coef_init_array,
                                                                                      dtype=tf.float32)
        ...
        output = weight_layers(1, x, mask, self.l2_coef, do_layer_norm=self.layer_norm,
                               ...
                               coef_init=coef_init, gamma_val=self.gamma_val,
                               trainable=self.trainable_coefs)["weighted_ops"][0]
        return output
    ...
    def __setstate__(self, state):
        ...
        if "coef_init_array" not in state:
            state["coef_init_array"] = None
        if "gamma_val" not in state:
            state["gamma_val"] = 1.0
        if "trainable_coefs" not in state:
            state["trainable_coefs"] = True
        super().__setstate__(state)


class ElmoWrapper(Configurable):
    def __init__(self, input_append: bool, output_append: bool, lm_reduce: SequenceMapper, rnn_layer: SequenceMapper):
        self.input_append = input_append
        self.output_append = output_append
        self.rnn_layer = rnn_layer
        self.lm_reduce = lm_reduce

    def apply(self, is_train, x, mask, lm):
        if self.input_append:
            with tf.variable_scope("input_lm"):
                input_lm = self.lm_reduce.apply(is_train, lm, mask)
            x = tf.concat([x, input_lm], axis=2)

        x = self.rnn_layer.apply(is_train, x, mask)

        if self.output_append:
            with tf.variable_scope("ouptut_lm"):
                output_lm = self.lm_reduce.apply(is_train, lm, mask)
            x = tf.concat([x, output_lm], axis=2)

        return x


def weight_layers(n_out_layers, lm_embeddings, mask, l2_coef=None,
                  use_top_only=False, do_layer_norm=True, coef_init=None, gamma_val=1.0, trainable=True):
    '''
        ...
        for k in range(n_out_layers):
            if use_top_only:
                ...
                W = tf.get_variable(
                    'ELMo_W_{}'.format(k),
                    shape=(n_lm_layers,),
                    initializer=coef_init or tf.zeros_initializer,
            ...
            gamma = tf.get_variable(
                ...
                shape=(1,),
                initializer=tf.constant_initializer(gamma_val, dtype=tf.float32),
                regularizer=None,
                trainable=trainable,
            )
