...
from hotpot.nn.ops import dropout
...
class CudnnRnnEncoder(_CudnnRnn):
    def map(self, is_train, x, mask=None):
        x = tf.transpose(x, [1, 0, 2])
        ...
        if self.bidirectional:
            with tf.variable_scope("forward"):
                fw = self._apply_transposed(is_train, x)[0]
                fw = tf.reverse_sequence(fw, mask, 0, 1)  # such that the first element is the final state of the rnn
            with tf.variable_scope("backward"):
                bw = self._apply_transposed(is_train, tf.reverse_sequence(x, mask, 0, 1))[0]
                bw = tf.reverse_sequence(bw, mask, 0, 1)  # here the first element is the final state of the reverse rnn
            out = tf.concat([fw[0], bw[0]], axis=1)
        else:
            fw = self._apply_transposed(is_train, x)[0]
            out = tf.reverse_sequence(fw, mask, 0, 1)[0]
        return out
...
class CudnnLstmEncoder(CudnnRnnEncoder, SequenceEncoder):
    def __init__(self,
                 n_units,
                 n_layers=1,
                 lstm_bias=1,
