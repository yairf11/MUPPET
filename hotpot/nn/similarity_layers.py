...
from hotpot.nn.layers import get_keras_initialization, get_keras_activation
...
def compute_attention_mask(x_mask, mem_mask, x_word_dim, key_word_dim):
    ...
    if x_mask is None and mem_mask is None:
        ...
        x_mask = x_mask if x_mask is not None else tf.ones(tf.shape(mem_mask)[0],  dtype=tf.int32)*x_word_dim
        mem_mask = mem_mask if mem_mask is not None else tf.ones(tf.shape(x_mask)[0], dtype=tf.int32)*key_word_dim
...
class SimilarityFunction(Configurable):
    ...
    def get_one_sided_scores(self, tensor_1, tensor_2):
        return tf.squeeze(self.get_scores(tf.expand_dims(tensor_1, 1), tensor_2), axis=[1])
