...
from hotpot.nn.layers import AttentionMapper, MergeLayer, SequenceEncoder, get_keras_initialization, SequenceMapper, \
    Mapper, SequenceMultiEncoder
from hotpot.nn.ops import VERY_NEGATIVE_NUMBER, exp_mask
from hotpot.nn.similarity_layers import SimilarityFunction, compute_attention_mask
...
class AttentionWithPostMapper(AttentionMapper):
    """ A wrapper for an attention mechanism followed by a sequence mapper """

    def __init__(self, attention: AttentionMapper, post_mapper: Optional[SequenceMapper]):
        self.attention = attention
        self.post_mapper = post_mapper

    def apply(self, is_train, x, keys, memories, x_mask=None, memory_mask=None):
        with tf.variable_scope('attention'):
            x = self.attention.apply(is_train, x, keys, memories, x_mask, memory_mask)

        if self.post_mapper is not None:
            with tf.variable_scope('post_mapper'):
                x = self.post_mapper.apply(is_train, x, x_mask)
...
class BiAttention(AttentionMapper):
    ...
    def apply(self, is_train, x, keys, memories, x_mask=None, memory_mask=None):
        ...
        joint_mask = compute_attention_mask(x_mask, memory_mask, x_word_dim, key_word_dim)
        if joint_mask is not None:
            dist_matrix += VERY_NEGATIVE_NUMBER * (1 - tf.cast(joint_mask, dist_matrix.dtype))
...
class ManyToOne(SequenceMapper):
    """
    This is a wrapper class for performing attention from multiple sequences to a single one.
    In our case, the single sequence will most likely be the question,
        while the other sequences are the multiple contexts of the question.
    It takes a regular sequence-to-sequence attention mechanism, and applies it to each question-context pair.
    Then, it combines the attended sequences to a single one by a layer given as input.
    """

    def __init__(self, attention: AttentionMapper, reduce: str):
        self.attention = attention
        self.reduce = reduce
